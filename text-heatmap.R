...
# Tokenize the text into words and calculate frequencies
word_counts <- tibble(line = 1, text = paragraph) %>%
...
ggplot(word_counts, aes(x = reorder(word, n), y = n)) +
  ...
  coord_flip() +
  labs(title = "Word Frequency Heatmap", x = "Words", y = "Frequency") +
  theme_minimal()


# Install and load necessary libraries
install.packages(c("tm", "ggplot2", "dplyr", "reshape2"))
library(tm)
library(ggplot2)
library(dplyr)
library(reshape2)
...
# Step 5: Select the top N words for the heat map
top_n <- 20  # Change this value for more or fewer words
top_words <- head(word_freq_df, top_n)

# Step 6: Create the heat map data frame
heatmap_data <- data.frame(word = top_words$word, freq = top_words$freq)

# Step 7: Create a heatmap
ggplot(heatmap_data, aes(x = word, y = 1, fill = freq)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = "Heat Map of Word Frequencies in Chronicles of Narnia",
       x = "Words", y = "Frequency") +
  theme_minimal() +
  geom_text(aes(label = freq), vjust = 1)  # Optional: Add frequency labels
