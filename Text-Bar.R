...
# Filter the most frequent words (you can adjust the number)
top_words <- word_counts %>% 
  filter(n > 2)  # Display words with more than 2 occurrences

# Create the bar plot using ggplot2
ggplot(top_words, aes(x = reorder(word, -n), y = n)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  theme_minimal() +
  labs(title = "Word Frequency in the Paragraph", 
       x = "Words", 
       y = "Frequency") +
...
# Step 5: Create a bar plot for the word frequencies
# Select the top 10 most frequent words for the plot
top_n <- 100
...
barplot(top_words,
        ...
        ylab = "Frequency",
        col = "purple",
        names.arg = names(top_words),
        las = 2,  # Make the x-axis labels vertical
