...
# Step 5: Create a rank for the words based on frequency
word_freq_df$rank <- 1:nrow(word_freq_df)

# Step 6: Create a scatter plot of word frequencies vs. their ranks
ggplot(word_freq_df, aes(x = rank, y = freq)) +
  geom_point(color = "lightblue", size = 3) +
  labs(title = "Scatter Plot of Word Frequencies vs. Ranks",
       x = "Rank of Words", y = "Word Frequency") +
  theme_minimal()
