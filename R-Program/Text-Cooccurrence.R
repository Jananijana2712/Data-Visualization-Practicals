...
install.packages(c("tm", "tidytext", "dplyr", "widyr", "reshape2", "ggplot2"))
library(tm)
library(tidytext)
library(dplyr)
library(widyr)
...
# Step 2: Preprocess the text (convert to lowercase, remove punctuation, stop words)
corpus <- tm_map(corpus, content_transformer(tolower))
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, removeWords, stopwords("en"))

# Step 3: Convert to a data frame for analysis
...
# Step 5: Create bigrams (word pairs)
bigrams <- tokens %>%
  mutate(next_word = lead(word)) %>%
  filter(!is.na(next_word))

# Step 6: Count co-occurrences of words
co_occurrence <- bigrams %>%
  count(word, next_word, sort = TRUE)

# Step 7: Create a term co-occurrence matrix
co_occurrence_matrix <- co_occurrence %>%
  spread(key = next_word, value = n, fill = 0)

# Step 8: Convert the matrix to a numeric matrix for plotting
co_occurrence_matrix <- as.matrix(co_occurrence_matrix[,-1])  # Remove the word column

# Step 9: Plot heatmap for the term co-occurrence matrix
ggplot(melt(co_occurrence_matrix), aes(Var1, Var2, fill = value)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = "Term Co-occurrence Matrix for Chronicles of Narnia",
       x = "Word", y = "Co-occurring Word") +
  theme_minimal() +
