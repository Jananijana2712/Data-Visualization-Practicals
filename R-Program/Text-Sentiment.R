...
install.packages(c("tm", "syuzhet", "ggplot2", "dplyr", "tidyr"))
library(tm)
library(syuzhet)
library(ggplot2)
library(dplyr)
...
sentiment_scores <- get_nrc_sentiment(as.character(corpus))
sentiment_data <- data.frame(t(sentiment_scores))

# Step 4: Add a column for the sentiment types
sentiment_data$Sentiment <- rownames(sentiment_data)

# Step 5: Reshape the data for plotting
sentiment_data_long <- sentiment_data %>%
  gather(key = "Sentiment_Type", value = "Score", -Sentiment)

# Step 6: Create a sentiment analysis plot
ggplot(sentiment_data_long, aes(x = Sentiment, y = Score, fill = Sentiment_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Sentiment Analysis of Chronicles of Narnia",
       x = "Sentiment", y = "Score") +
  theme_minimal() +
