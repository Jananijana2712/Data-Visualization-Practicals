...
# Step 5: Select the top N words for the pie chart
top_n <- 100  # Change this value for more or fewer words
top_words <- head(word_freq_df, top_n)

# Step 6: Create the pie chart
ggplot(top_words, aes(x = "", y = freq, fill = word)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Word Frequency Pie Chart for Chronicles of Narnia",
       fill = "Words") +
  theme_void() +
...
# Step 5: Select the top N words for the pie chart
top_n <- 50  # Change this value for more or fewer words
top_words <- head(word_freq_df, top_n)

# Step 6: Create the pie chart with frequencies
ggplot(top_words, aes(x = "", y = freq, fill = word)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Pie Chart- Chronicles of Narnia",
       fill = "Words") +
  theme_void() +
  theme(legend.position = "right") +
  geom_text(aes(label = paste(word, " (", freq, ")", sep = "")), 
