...
ggplot(words_df, aes(x = word_length)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Histogram of Word Lengths",
       x = "Word Length",
       y = "Frequency") +
  theme_minimal()
...
# Step 2: Preprocess the corpus (convert to lowercase, remove punctuation, and remove stopwords)
...
# Step 6: Create a data frame of word frequencies
word_freq_df <- data.frame(word = names(word_freq), freq = word_freq)

# Step 7: Plot histogram of word frequencies
ggplot(data = word_freq_df, aes(x = word, y = freq)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Histogram of Word Frequencies in Chronicles of Narnia",
       x = "Words",
       y = "Frequency") +
