...
split <- sample.split(dataset$COMP, SplitRatio = 0.7)  # 70% training, 30% testing
train_data <- subset(dataset, split == TRUE)
test_data <- subset(dataset, split == FALSE)

svm_model <- svm(COMP ~ TP2 + TP3 + H1 + DV_pressure + Oil_temperature + Motor_current + DV_eletric + Towers + MPG + LPS + Pressure_switch + Oil_level + Caudal_impulses, data = train_data, kernel = "radial", probability = TRUE) 

predicted_classes <- predict(svm_model, test_data)
predicted_probabilities <- attr(predict(svm_model, test_data, probability = TRUE), "probabilities")[,2]                
